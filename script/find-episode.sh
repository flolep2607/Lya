...
#TVSHOW=$(echo "$query" | awk -F ' ' '{print $1}')
NUM=$(echo "$query" | awk -F ' ' '{print $NF}')
TVSHOW=${query::-${#NUM}}

#echo "NUM: $NUM"

EPISODE=$(echo "$NUM" | sed 's/.*\(..\)/\1/')
SEASON=$(echo "$NUM" | sed 's/\(.\{1,2\}\)../\1/')
...
PATH="/var/media/krokodin/Serie/"

echo "Episode: $EPISODE -- Season: $SEASON -- TVShow: $TVSHOW"
...
RETURN=$(/usr/bin/ssh -T root@192.168.1.25 'find '"$PATH"' -iname "*'"$TVSHOW"'*" -type f | grep -i "S'"$SEASON"'" | grep "E[P]*'"$EPISODE"'" | grep -v ".srt"')
echo "return: $RETURN"
